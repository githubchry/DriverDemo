...
static ssize_t gpio_demo_read(struct file *flip, char __user *buf, size_t len, loff_t *pos)
{
    ...
    data[0] = '0' + gpio_get_value(gpio_demo_dev->gpio);
    //这里使用了最新的gpiod的接口获取当前gpio的方向
    data[1] = '0' + gpiod_get_direction(gpio_to_desc(gpio_demo_dev->gpio));
    data[2] = '\n';     //换行符 cat的时候打印好看一点
...
{   
    ...
    {
    case '0':
        ret = gpio_direction_output(gpio_demo_dev->gpio, 0); 
        break;
    case '1':
        ret = gpio_direction_output(gpio_demo_dev->gpio, 1); 
        break;
    case '2':
        // 设置gpio输入输出方向为输入
        ret = gpio_direction_input(gpio_demo_dev->gpio); 
        break;
    default:
        return -EINVAL;
        break;
...
static int __init gpio_demo_init(void)
{
    ...
    //7.导出gpio248 相当于echo 7 > /sys/class/gpio/export
    gpio_demo_dev->gpio = RK3288_GPIO0_A7;
    ret = gpio_request(gpio_demo_dev->gpio, NAME);
    if(ret != 0)
    {
        printk(KERN_ERR "%s,%s:%d gpio_request[%d] failed %d \n", __FILE__, __func__, __LINE__, gpio_demo_dev->gpio, ret);
        goto err5;
    }
    ...
    */
    ...
    // 设置为低电平   echo 0 > value
    gpio_set_value(gpio_demo_dev->gpio, 0); 
    msleep_interruptible(500);

    // 设置为高电平   echo 1 > value
    gpio_set_value(gpio_demo_dev->gpio, 1); 

    printk(KERN_INFO "%s,%s:%d ojbk\n", __FILE__, __func__, __LINE__);
    return 0;

err6:
    gpio_free(gpio_demo_dev->gpio);
err5:
